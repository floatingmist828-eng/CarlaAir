...
class ActiveUAVTaskApp:
    ...
    def setup(self) -> None:
        if self.client is None or self.world is None:
            self.connect()
        cleanup_actors_by_role(self.world, {"ego", "task_traffic", "task_ego", "task_uav"})
        ...
        if self._ego_control_mode in {"route_follow", "route", "behavior", "vision_simple", "vision_rgb_only"}:
            try:
                self.ego_vehicle.set_autopilot(False)
            except Exception:
                pass
            if self._ego_control_mode in {"vision_simple", "vision_rgb_only"}:
                self.ego_driver = VisionEgoDriver(
                    self.world,
                    self.ego_vehicle,
                    target_speed_mps=float(self.scenario.ego_target_speed_mps),
                    use_semantic=self._ego_control_mode != "vision_rgb_only",
        ...
        self.traffic_actors = [
            item.actor for item in spawn_traffic_vehicles(
                self.client,
                self.world,
