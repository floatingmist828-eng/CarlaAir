...
data_status="$LOG_DIR/smoke_25m_vehicle_supervisor_data_status.json"
latest_log="$LOG_DIR/smoke_25m_vehicle_supervisor.latest"

check_data_ready() {
  python scripts/griffin_repro.py check-data-packages --dataset 50scenes_25m --package-profile smoke_25m_vehicle --json > "$data_status"
...
cleanup_stale_downloads() {
  ...
  pkill -f "bash griffin_repro/download_50scenes_25m_vehicle_mobaxterm.sh" 2>/dev/null || true
}
...
while true; do
  ...
  cleanup_stale_downloads
  set +e
  bash griffin_repro/download_50scenes_25m_vehicle_mobaxterm.sh
  download_status=$?
  set -e
...
smoke_log="$LOG_DIR/smoke_25m_vehicle_supervisor_$(date +%Y%m%d_%H%M%S).log"
echo "$smoke_log" > "$latest_log"
bash griffin_repro/run_smoke_25m_vehicle_mobaxterm.sh 2>&1 | tee "$smoke_log"
