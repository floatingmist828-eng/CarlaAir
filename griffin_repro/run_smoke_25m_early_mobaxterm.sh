...
if [ "$partial_scene_limit" -gt 0 ]; then
  python scripts/griffin_repro.py materialize-partial-images --profile smoke_25m_early --image-side vehicle-side "${partial_args[@]}" --json | tee "$LOG_DIR/smoke_25m_early_vehicle_partial_images.json"
  python scripts/griffin_repro.py materialize-partial-images --profile smoke_25m_early --image-side drone-side "${partial_args[@]}" --json | tee "$LOG_DIR/smoke_25m_early_drone_partial_images.json"

  drone_query_json="$LOG_DIR/smoke_25m_early_drone_query_partial_eval.json"
  python scripts/griffin_repro.py prepare-drone-query-partial-eval --profile smoke_25m_early "${partial_args[@]}" --json | tee "$drone_query_json"
